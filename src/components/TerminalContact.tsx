...
const TerminalContact: React.FC = () => {
  ...
  return (
    <div className="terminal w-full max-w-2xl mx-auto h-80 overflow-y-auto">
      <div className="mb-4">
        {output.map((line, index) => (
          <p key={index} className="font-pixel text-xs leading-relaxed">{line}</p>
        ))}
      </div>
      <form onSubmit={handleSubmit} className="flex">
        <span className="font-pixel text-xs mr-2 text-gameboy-accent">{">"}</span>
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          className="flex-1 bg-transparent border-none outline-none text-gameboy-accent font-pixel text-xs"
          autoFocus
        />
        <span className={`h-4 w-2 bg-gameboy-accent ${showCursor ? "opacity-100" : "opacity-0"} transition-opacity`}></span>
      </form>
    </div>
  );
};
